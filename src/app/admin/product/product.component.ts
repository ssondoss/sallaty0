...
export class ProductComponent implements OnInit {
  ...
  public onAddProduct(values: Object) {
    if (this.newProductForm.valid)
      this.http.post(this.URL, this.newProductForm.value).subscribe({
        next: response => {
          this.snackBar.open("New Product was Created Successfully!", "×", {
            panelClass: "success",
        ...
        error: (response: HttpErrorResponse) => {
          this.snackBar.open(
            "Sorry , There wan an error while creating the new Product !",
            "×",
      ...
      this.snackBar.open("all the fields are required", "x", {
        panelClass: "error",
        verticalPosition: "top",
        duration: 3000
      });
  ...
  public deleteProduct(product: any): void {
    this.http.delete(this.URL + "/" + product.id).subscribe({
      ...
      error: (response: HttpErrorResponse) => {
        this.snackBar.open(
          "Sorry , There wan an error while deleting the Product !",
          "×",
          { panelClass: "error", verticalPosition: "top", duration: 3000 }
        );
      }
  ...
  private getCategories(): void {
    this.http.get(environment.apiUrl + "/category").subscribe(data => {
